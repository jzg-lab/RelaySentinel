...
负责登录、配置管理、上游中转管理、自有中转站/号池管理、通知渠道、手动检查、移动端工作台数据和告警处理动作。
...
- 上游中转适配器：New API、Sub2API。
- 号池巡检适配器：New API、Sub2API。
- 高级号池额度来源：CLIProxyAPI/CPA，只在检测到可用 Management API 或配套额度服务时启用。

上游中转是老板向别人购买的外部服务；号池是老板自己的中转站或账号池。两者可以复用底层平台客户端，但不能在产品模型和接口入口上混成一个“监控目标”。适配器只负责把平台响应转换成统一的 `QuotaSignal` 或 `PoolSnapshot`。规则引擎不应该知道各平台的原始字段。
...
- `upstream_targets`：外部上游中转，仅 New API/Sub2API，保存名称、平台、地址、余额状态、阈值。
- `pool_targets`：自己的中转站或账号池，默认 New API/Sub2API，保存名称、平台、账号健康和额度巡检状态。
- `pool_quota_sources`：号池高级额度来源，例如 CLIProxyAPI/CPA 的 Management API 或配套额度服务。
...
- 上游页的添加入口叫“添加上游”，只提供 New API/Sub2API。
- 号池页的添加入口叫“添加自己的中转站”，默认只提供 New API/Sub2API；CLIProxyAPI/CPA 放在高级额度来源里，不放进上游入口。
